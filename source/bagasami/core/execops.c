...
static void fatopen(ASMSys *system){
    ...
    system->fh[handle] = fopen(fileName, mode);
...
static void fatremove(ASMSys *system){
    ...
    FilePath *path = BASM_CheckPathLocality(system, file);
    char *fileName = FilePath_Str(path);
    system->cpu.curVal = (!remove(fileName));
    FilePath_Destroy(path);
    return;
}
...
static void fatrename(ASMSys *system){
    ...
    FilePath *path1 = BASM_CheckPathLocality(system, oldPath);
    char *fileName1 = FilePath_Str(path1);
    ...
    FilePath *path2 = BASM_CheckPathLocality(system, newPath);
    char *fileName2 = FilePath_Str(path2);
    //rename the file
    system->cpu.curVal = (!rename(fileName1, fileName2));

    FilePath_Destroy(path1);
    FilePath_Destroy(path2);
    return;
}
