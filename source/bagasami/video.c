...
static void _gfxSetPix(ASMSys *system){
    //dest buffer, buffer wd, buffer ht, xpos, ypos, color
    unsigned short *dest = (unsigned short *)*system->cpu.arg[0];


    int wd = (int)*system->cpu.arg[1];
    int ht = (int)*system->cpu.arg[2];
    int x = (int)*system->cpu.arg[3];
    int y = (int)*system->cpu.arg[4];
    //make sure we aren't writing out of bounds anywhere
    x = (x < 0) ? 0 : x;
    y = (y < 0) ? 0 : y;
    x = (x >= wd) ? wd - 1 : x;
    y = (y >= ht) ? ht - 1 : y;
...
==============================================*/
  ...
  static void _loadSpr(ASMSys *system){
    ...
    FilePath *path = BASM_CheckPathLocality(system, file);

    char *fileName = FilePath_Str(path);

  	if(BAG_Display_LoadObj(fileName, system->video.sprite[sprite_num]) != ERR_NONE){
      if(GET_FLAG(system->flags, ASM_AUTOERROR))
  		  errorMsg(system, system->prgm.pos, "Missing sprite: %s\n", fileName);//eventually remove this
...
==============================================*/
  ...
  static void _loadBG(ASMSys *system){
    ...
    FilePath *path = BASM_CheckPathLocality(system, file);
    char *fileName = FilePath_Str(path);
    if(BAG_TileBG_LoadBG(fileName, system->video.background[bg_num]) != ERR_NONE){
      if(GET_FLAG(system->flags, ASM_AUTOERROR)){
  ...
  static void _drawBG(ASMSys *system){
    ...
    //vidQNode temp = {bg_num, QUEUE_TYPE_BG, 0, wd, ht, (void*)dest};
    vidQNode temp = {bg_num, QUEUE_TYPE_BG, 0, SCREEN_WIDTH, SCREEN_HEIGHT, (void*)dest};
    //videoQueue_Add(system->video.gfxQue, &temp, NULL);


    //vidQNode temp = {id, QUEUE_TYPE_SPR, 0, wd, ht, (void*)dest};
    videoQueue_Add(system->video.gfxQue, &temp, system->video.background[bg_num]);
  ...
  static void _loadFont(ASMSys *system){
    ...
    FilePath *path = BASM_CheckPathLocality(system, file);
    char *fileName = FilePath_Str(path);
    if(BAG_Font_Load(fileName, system->video.font[slot]) != ERR_NONE){
      if(GET_FLAG(system->flags, ASM_AUTOERROR)){
...
static void _drawAll(ASMSys *system, videoQueue *qSys){
 ...
 static void _flip(ASMSys *system){
  //grab arguments
  bagUWord scrn = (bagUWord)(*system->cpu.arg[0]);
