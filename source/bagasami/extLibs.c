...
#if defined(BASM_USE_EXTLIB_INI)
/**************************************************************************************
INI file reading/writing
**************************************************************************************/
static void _iniOpen(ASMSys *system){
	//arguements
	char *file = (char*)system->cpu.arg[0];
	...
	if(system->iniFile.load(&system->iniFile, fileName, INI_SORT | INI_ADDMISSING) < 0){
...
static void _iniWrite(ASMSys *system){
  	if(system->iniFile.write){
		...
		//arguments
		//output filepath
		system->iniFile.write(&system->iniFile, fileName);
		FilePath_Destroy(path);
  	}
}
...
static void _scriptExecute(ASMSys *system) {
	...
	//if an absolute path was given, use it
	FilePath *path = BASM_CheckPathLocality(system, (char*)system->cpu.arg[0]);
    ...
    if(argc > 0) {
        ...
        System_PushScript(path, system, SCRIPT_RETURNTOPARENT, argv, argc);
    } else {
        System_PushScript(path, system, SCRIPT_RETURNTOPARENT, NULL, 0);
    }
    FilePath_Destroy(path);
